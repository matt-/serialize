(function () {
      ...
      if(obj.hasOwnProperty(key)) {
        if(typeof obj[key] === 'object') {
          ...
          if(obj[key].indexOf(FUNCFLAG) === 0) {
            functionStr = '(' + obj[key].substring(FUNCFLAG.length) + ')';
            if(valadateFunction(functionStr)){
                obj[key] = eval(functionStr);
            }else{
                throw new Error('Can\'t deserialize executable JavaScript code.');
            }
  ...
  function valadateFunction(functionStr){
      var ast = esprima.parse(functionStr);
      return ast.body[0].expression.type == 'FunctionExpression';
  }
